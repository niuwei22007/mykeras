...
class InhibitUnit(Callback):
    def __init__(self, filepath, monitor='val_loss', verbose=0, save_best_only=False, scale=0.2):
    ...
    # def on_batch_end(self, batch, logs={}):
        # print('****************************************************************')
        # self.model.inhibit_weights(scale=self.scale)

    def on_epoch_end(self, epoch, logs={}):
        print('****************************************************************')
        self.model.inhibit_weights(scale=self.scale)
        print('inhibit weight with rate is', self.scale)
    # def on_epoch_end(self, epoch, logs={}):
    #     filepath = self.filepath.format(epoch=epoch, **logs)
    #     if self.save_best_only:
    #         current = logs.get(self.monitor)
    ...
    #         self.model.inhibit_weights(scale=self.scale)
            # print('----------------------------------------------------------------')
            # self.model.inhibit_weights_Save(filepath, overwrite=True, scale=self.scale)
            # print('----------------------------------------------------------------')


class ModelCheckBegin(Callback):
    def __init__(self, filepath, monitor='loss', verbose=0, save_best_only=False):
