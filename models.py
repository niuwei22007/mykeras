...
class Sequential(Model, containers.Sequential):
    ...
    def compile(self, optimizer, loss,
        ...
        self.my_y_test = self.get_output(train=True)
        ...
        elif class_mode == "autoencoder":
            train_accuracy = K.sum(K.square(self.y - self.y_train))
            test_accuracy = K.sum(K.square(self.y - self.y_test))
        else:
        ...
        self._mypredict = K.function(predict_ins, [self.my_y_test], updates=self.state_updates)
    ...
    def mypredict(self, X, batch_size=128, verbose=0):
        X = standardize_X(X)
        return self._predict_loop(self._mypredict, X, batch_size, verbose)[0]
