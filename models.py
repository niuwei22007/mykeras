...
class Sequential(Model, containers.Sequential):
    ...
    def inhibit_weights_Save(self, filepath, overwrite=False, scale=0.2):
        # Save weights from all layers to HDF5
        import h5py
        import os.path
        # if file exists and should not be overwritten
        ...
        for k, l in enumerate(self.layers):
            ...
            for n, param in enumerate(weights):
                ...
                param_dset = g.create_dataset(param_name, param.shape, dtype=param.dtype)
                absp = abs(param)
                param[absp<(absp.max()-absp.min())*scale]=0
                newWeights += [param]
                param_dset[:] = param
            l.set_weights(newWeights)
        f.flush()
        f.close()
        ...
        for k, l in enumerate(self.layers):
            # if k < (lenth-1):
                ...
                for n, param in enumerate(weights):
                    ...
                    newWeights += [param]
                l.set_weights(newWeights)
        #print('inhibit model rate is', scale)
